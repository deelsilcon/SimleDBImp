...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int cardinality;
        // some code goes here
        // simply implement as the doc implies
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        Set<Set<LogicalJoinNode>> nodes;
        for (int i = 1; i <= joins.size(); i++) {
            nodes = enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> subNodes : nodes) {
                CostCard bestPlan = new CostCard();
                bestPlan.cost = Double.MAX_VALUE;
                for (LogicalJoinNode node : subNodes) {
                    ...
                    if (c != null) {
                        bestPlan = c;
                        planCache.addPlan(subNodes, bestPlan.cost, bestPlan.card, bestPlan.plan);
                    }
                }
            }
        }
        if (explain) {
            printJoins(planCache.getOrder(new HashSet<>(joins)), planCache, stats, filterSelectivities);
        }
