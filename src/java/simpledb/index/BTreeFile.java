...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if (category == BTreePageId.LEAF) {
        ...
        } else if (category == BTreePageId.INTERNAL) {
            BTreeEntry entry;
            ...
            //check the existence of valid entry
            if (internalPageIterator.hasNext()) {
                entry = internalPageIterator.next();
            } else {
                throw new DbException("[Error]:No entry in this node!");
            }
            BTreePageId next;
            if (f == null) {
        ...
        throw new DbException("Can't find wanted page in this b+ tree");
    }
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        // copy half of the tuples to the right sibling
        Iterator<Tuple> it = page.reverseIterator();
        int totalTuples = page.getNumTuples();
        int half = totalTuples / 2;
        for (int i = 0; i < half; ++i) {
            assert it.hasNext() : "Iterator error!";
            Tuple t = it.next();
            page.deleteTuple(t);
            rightSibling.insertTuple(t);
        }

        // update the origin right sibling's pointers if necessary
        if (page.getRightSiblingId() != null) {
            BTreeLeafPage right = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
            right.setLeftSiblingId(rightSibling.getId());
            rightSibling.setRightSiblingId(right.getId());
            dirtypages.put(right.getId(), right);
        }
        rightSibling.setLeftSiblingId(page.getId());
        page.setRightSiblingId(rightSibling.getId());

        // update parent, copy the middle key up. The middle key is the smallest element
        // in the right sibling
        Field middleKey = rightSibling.getTuple(0).getField(keyField);
        BTreeEntry entry = new BTreeEntry(middleKey, page.getId(), rightSibling.getId());
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), middleKey);
        parent.insertEntry(entry);

        // note that we don't need to call setParentId() separately
        updateParentPointers(tid, dirtypages, parent);
        // put this two dirty page into map
        dirtypages.put(page.getId(), page);
        dirtypages.put(rightSibling.getId(), rightSibling);
        return middleKey.compare(Op.GREATER_THAN_OR_EQ, field) ? page : rightSibling;
    }
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // should be inserted.
        BTreeInternalPage rightSibling = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);

        // Note that the current iterator iter over entries instead of tuples
        Iterator<BTreeEntry> it = page.reverseIterator();
        int totalTuples = page.getNumEntries();
        int half = totalTuples / 2;
        for (int i = 0; i < half; ++i) {
            assert it.hasNext() : "Iterator error!";
        ...
        //Basically the same as B+ tree leaf page, except that we need to push the middle key up
        //(i.e. delete the key in the child)
        BTreeEntry middleEntry = it.next();
        Field middleKey = middleEntry.getKey();
        page.deleteKeyAndRightChild(middleEntry);
        middleEntry = new BTreeEntry(middleKey, page.getId(), rightSibling.getId());
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int nTuplesInPage = page.getNumTuples();
        int nTuplesInSib = sibling.getNumTuples();
        while (nTuplesInPage < nTuplesInSib) {
            Tuple tupleToMove;
            if (isRightSibling) {
                tupleToMove = sibling.iterator().next();
            } else {
                tupleToMove = sibling.reverseIterator().next();
            }
            sibling.deleteTuple(tupleToMove);
            page.insertTuple(tupleToMove);
            // Update parent entry !!!
            entry.setKey(tupleToMove.getField(keyField));
            parent.updateEntry(entry);

            nTuplesInPage = page.getNumTuples();
            nTuplesInSib = sibling.getNumTuples();
        }
    }
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        int nSibEntries = leftSibling.getNumEntries();
        int nPageEntries = page.getNumEntries();
        // Draw a graph will help a lot when coding this part
        while (nPageEntries < nSibEntries) {
            BTreeEntry leftSibEntry = leftSibling.reverseIterator().next();
            BTreeEntry pageEntry = page.iterator().next();

            BTreeEntry entry = new BTreeEntry(
                    parentEntry.getKey(),
                    leftSibEntry.getRightChild(),
                    pageEntry.getLeftChild());

            page.insertEntry(entry);

            parentEntry.setKey(leftSibEntry.getKey());
            parent.updateEntry(parentEntry);

            leftSibling.deleteKeyAndRightChild(leftSibEntry);
            // We don't need to update the left sibling's pointers because we call the delete method above
            updateParentPointers(tid, dirtypages, page);
            ...
            nPageEntries = page.getNumEntries();
            nSibEntries = leftSibling.getNumEntries();
        }
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // the corresponding parent entry. Be sure to update the parent
        // pointers of all children in the entries that were moved.
        int nSibEntries = rightSibling.getNumEntries();
        int nPageEntries = page.getNumEntries();

        while (nPageEntries < nSibEntries) {
            BTreeEntry pageEntry = page.reverseIterator().next();
            BTreeEntry rightSibEntry = rightSibling.iterator().next();
            BTreeEntry entry = new BTreeEntry(
                    parentEntry.getKey(),
                    pageEntry.getRightChild(),
                    rightSibEntry.getLeftChild());

            page.insertEntry(entry);

            parentEntry.setKey(rightSibEntry.getKey());
            parent.updateEntry(parentEntry);

            rightSibling.deleteKeyAndLeftChild(rightSibEntry);
            updateParentPointers(tid, dirtypages, page);
            ...
            nPageEntries = page.getNumEntries();
            nSibEntries = rightSibling.getNumEntries();
        }
    }
    ...
    public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        if(rightPage.getRightSiblingId() != null) {
            ...
            BTreeLeafPage right = (BTreeLeafPage) getPage(tid, dirtypages, id, Permissions.READ_WRITE);

            leftPage.setRightSiblingId(right.getId());
