                ...
                while (raf.getFilePointer() < logFile.length()) {
                    ...
                    switch (type) {
                        ...
                        case UPDATE_RECORD:
                            ...
                            readPageData(raf);
                            if (id == tid.getId()) {
                                Database.getBufferPool().restorePage(before);
                            }
                            raf.readLong();
                            break;
                        default:
...
//                raf.seek(logFile.length() - LONG_SIZE);
//                long start = raf.readLong();
//                while (start > tidToFirstLogRecord.get(tid.getId())) {
//                    raf.seek(start);
//                    int type = raf.readInt();
//                    long id = raf.readLong();
...
//                            readPageData(raf);
//                            if (tid.getId() == id) {
//                                Database.getBufferPool().restorePage(beforePage);
//                            }
//                            break;
//                        case BEGIN_RECORD:
//                            if(tid.getId() == id){
...
//                }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                recoveryUndecided = false;
                // find the index of checkpoint first
                raf.seek(0);
                long index = raf.readLong();
                List<Long> undoList;
                if (index == -1) {
                    undoList = redoPhase(0 + LONG_SIZE, false);
                } else {
                    undoList = redoPhase(index, true);
                }
                undoPhase(undoList);
            }
        }
    ...
    private List<Long> redoPhase(long start, boolean isCheckPoint) throws IOException {
        List<Long> undoList = new ArrayList<>();
        raf.seek(start);
        if (isCheckPoint) {
            int type = raf.readInt();
            long id = raf.readLong();
            assert type == CHECKPOINT_RECORD : "[Error] Wrong log type! Should be of type CHECKPOINT_RECORD";
            int numOfTxns = raf.readInt();
            while (numOfTxns-- > 0) {
                long tid = raf.readLong();
                raf.readLong();
                undoList.add(tid);
...
//            System.out.println(prev);
            switch (type) {
                case UPDATE_RECORD:
                    ...
                    Database.getBufferPool().restorePage(afterImage);
                    raf.readLong();
                    break;
                case COMMIT_RECORD:
                    undoList.remove(id);
    ...
    private void undoPhase(List<Long> undoList) throws IOException {
        raf.seek(logFile.length() - LONG_SIZE);
        long start = raf.readLong();
...
//            System.out.println(start);
    ...
    private void doAbortInRedo(long tid, long index) throws IOException {
        long start = index;
        System.out.println(start);
        boolean jumpOut = false;
        while (true) {
